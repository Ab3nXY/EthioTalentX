import json
from django.contrib.auth.models import User  # Assuming you're using User model directly
from channels.generic.websocket import AsyncWebsocketConsumer
from .models import ChatRoom, ChatMessage  # Import your chat app models
from django.core.files import async_to_sync
...
class ChatConsumer(AsyncWebsocketConsumer):
    async def connect(self):
        ...
        if not self.room_name:
            ...
            new_room, created = ChatRoom.objects.get_or_create(name=new_room_name)
    ...
    async def receive(self, text_data):
        ...
        try:
            chat_room = await async_to_sync(ChatRoom.objects.get, pk=room_id)  # Get the chat room
        ...
        chat_message = ChatMessage.objects.create(
            sender=self.user,
            room=ChatRoom.objects.get(name=self.room_name),  # Assuming room has a name field
            message=message
        )
    ...
    async def send_message(self, event):
        ...
        await self.channel_layer.group_send(
            str(room_id),  # Convert room ID to string for group name
            {
                "type": "send_message",
                "message": message,
                "username": username,
